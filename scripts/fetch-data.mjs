...
import fs from 'fs';
/* ---------- passphrase crypto (AES-256-GCM, PBKDF2-SHA256 250k) ---------- */
const PASS = process.env.DATA_PASSPHRASE;
if(!PASS){ console.error('DATA_PASSPHRASE secret not set — aborting.'); process.exit(1); }
const te=s=>new TextEncoder().encode(s);
const b64e=u8=>Buffer.from(u8).toString('base64');
const b64d=s=>new Uint8Array(Buffer.from(s,'base64'));
async function deriveKey(pass,salt){
  const km=await crypto.subtle.importKey('raw',te(pass),'PBKDF2',false,['deriveKey']);
...
async function encryptJson(obj,pass){
  ...
  const ct=new Uint8Array(await crypto.subtle.encrypt({name:'AES-GCM',iv},key,te(JSON.stringify(obj))));
  return JSON.stringify({v:1,salt:b64e(salt),iv:b64e(iv),ct:b64e(ct)});
}
async function decryptJson(envStr,pass){
  const env=JSON.parse(envStr);
  const key=await deriveKey(pass,b64d(env.salt));
  const pt=await crypto.subtle.decrypt({name:'AES-GCM',iv:b64d(env.iv)},key,b64d(env.ct));
  return JSON.parse(new TextDecoder().decode(pt));
}
if(!fs.existsSync('config.enc')){
  console.log('config.enc not found — setup pending (save profiles from the dashboard first). Exiting cleanly.');
  process.exit(0);
}
let CONFIG;
try{ CONFIG=await decryptJson(fs.readFileSync('config.enc','utf8'), PASS); }
catch(e){ console.error('Cannot decrypt config.enc — DATA_PASSPHRASE secret does not match the dashboard passphrase.'); process.exit(1); }
...
const main=async()=>{
  ...
  for(const profile of CONFIG.profiles){
    ...
    fs.writeFileSync('data-'+profile.slug+'.enc', await encryptJson(data, PASS));
    if(profile===CONFIG.profiles[0]) fs.writeFileSync('data.enc', await encryptJson(data, PASS));
